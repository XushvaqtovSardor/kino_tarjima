...
if [ -z "$1" ]; then
  ...
  echo ""
  echo "  Misol:"
  echo "  ./manual-restore.sh kino_tarjima_backup_20260222_100000.sql.gz"
  echo ""
  echo "🗂️  Mavjud backuplar:"
  ls -lht backups/kino_tarjima_backup_*.sql.gz 2>/dev/null | head -10 || echo "Backuplar topilmadi"
  exit 1
fi
...
if ! docker ps | grep -q kino_tarjima_db; then
  echo "❌ Xatolik: kino_tarjima_db container ishlamayapti!"
  echo ""
...
docker exec -i kino_tarjima_db /bin/bash <<EOF
set -e

echo "🔌 Database connectionlarni yopish..."
PGPASSWORD=kino_tarjima_pass_2026 psql -h localhost -U kino_tarjima_user -d postgres -c \
  "SELECT pg_terminate_backend(pid) 
   FROM pg_stat_activity 
   WHERE datname = 'kino_tarjima_db' AND pid <> pg_backend_pid();" >/dev/null 2>&1

echo "🗑️  Mavjud bazani o'chirish..."
PGPASSWORD=kino_tarjima_pass_2026 psql -h localhost -U kino_tarjima_user -d postgres -c \
  "DROP DATABASE IF EXISTS kino_tarjima_db;" >/dev/null 2>&1

echo "📦 Yangi baza yaratish..."
PGPASSWORD=kino_tarjima_pass_2026 psql -h localhost -U kino_tarjima_user -d postgres -c \
  "CREATE DATABASE kino_tarjima_db;" >/dev/null 2>&1

echo "📥 Ma'lumotlarni tiklash..."
gunzip -c "/backups/${BACKUP_FILE}" | PGPASSWORD=kino_tarjima_pass_2026 psql \
  -h localhost \
  -U kino_tarjima_user \
  -d kino_tarjima_db \
  -q \
...
echo ""
echo "📊 Statistika:"
TABLE_COUNT=\$(PGPASSWORD=kino_tarjima_pass_2026 psql -h localhost -U kino_tarjima_user -d kino_tarjima_db -t -c \
  "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='public';" | tr -d ' ')
echo "  • Jadvallar soni: \${TABLE_COUNT}"

USER_COUNT=\$(PGPASSWORD=kino_tarjima_pass_2026 psql -h localhost -U kino_tarjima_user -d kino_tarjima_db -t -c \
  "SELECT COUNT(*) FROM \\\"User\\\";" 2>/dev/null | tr -d ' ' || echo "0")
echo "  • Foydalanuvchilar: \${USER_COUNT}"

DB_SIZE=\$(PGPASSWORD=kino_tarjima_pass_2026 psql -h localhost -U kino_tarjima_user -d kino_tarjima_db -t -c \
  "SELECT pg_size_pretty(pg_database_size('kino_tarjima_db'));" | tr -d ' ')
